...
function GET_MISSED_HOURS(row: number): string {
  ...
  for (let col = resultSheet.getLastColumn(); col > currentWeekColIndex; col--) {
    let deltaMS: number = durationToDate(resultSheet.getRange(row, col).getDisplayValue()).getTime() - weeklyReqMS
    ...
    if (deltaMS < 0) {
      ...
      // Decrement requirement by additional hours over minimum requirement (final minimum of 0)
      requiredMS -= Math.min(deltaMS, requiredMS);
    }
  }

  // Decrement counter by additional hours logged in the current week
  const currentLoggedAboveMin: number = durationToDate(resultSheet.getRange(row, currentWeekColIndex).getDisplayValue()).getTime() - weeklyReqMS;
